<template>
  <div class="fireworks" v-show="show_firework">
    <!-- SVG Spritesheet -->
      ...
      <div class="stage-container remove">
        ...
        <div class="menu hide">
          <div class="menu__inner-wrap">
            ...
            <div class="menu__subheader cursor-pointer mt-4 font-bold" @click="handleFireworkBack">退回工作界面</div>
            <form>
...
export default {
  data() {
    return {
      userInfo: JSON.parse(localStorage.getItem('D_userInfo')),
      show_firework: false,
      env: getEnv()
    };
  },
  created() {
    if (!this.userInfo || this.env == 'test') return;
    if (this.userInfo.user_id != 20) return;
    this.loadCSS();
    this.loadJS();
  },
  methods: {
    handleFireworkBack() {
      this.$router.push({ name: '账号管理=>后台用户管理' });
      const timer = setTimeout(() => {
        this.$router.go(0);
      }, 3000);
    },
    loadCSS() {
      loadCSS('https://fireworks.nianbroken.top/NianBrokenStorage/fonts/css.css');
      loadCSS('https://fireworks.nianbroken.top/NianBrokenStorage/css/style.css');
    },
    loadJS() {
      loadJS('https://fireworks.nianbroken.top/NianBrokenStorage/js/fscreen.js');
      loadJS('https://fireworks.nianbroken.top/NianBrokenStorage/js/Stage.js');
      loadJS('https://fireworks.nianbroken.top/NianBrokenStorage/js/MyMath.js');
      loadJS('https://fireworks.nianbroken.top/NianBrokenStorage/js/script.js', () => {
